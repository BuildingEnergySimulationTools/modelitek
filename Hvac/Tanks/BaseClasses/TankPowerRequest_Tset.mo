...
block TankPowerRequest_Tset
  replaceable package Medium = Buildings.Media.Water "Medium model" annotation(
  ...
  // --- Entrées
  Modelica.Blocks.Interfaces.RealInput T_set "température cible [K]"annotation(
    Placement(transformation(origin = {6, 32}, extent = {{-150, 60}, {-100, 100}}), iconTransformation(origin = {10, 40}, extent = {{-150, 60}, {-100, 100}})));

  Modelica.Blocks.Interfaces.RealInput T_top "Température haut ballon [K]" annotation(
    Placement(transformation(extent = {{-140, 60}, {-100, 100}}), iconTransformation(extent = {{-140, 60}, {-100, 100}})));
  Modelica.Blocks.Interfaces.RealInput T_bot "Température bas ballon [K]" annotation(
    Placement(transformation(extent = {{-140, 20}, {-100, 60}})));
  Modelica.Blocks.Interfaces.RealInput m_flow "Débit PAC [kg/s]" annotation(
    Placement(transformation(extent = {{-140, -20}, {-100, 20}})));
  Modelica.Blocks.Interfaces.RealInput T_HP_out_max "Température max sortie PAC [K]" annotation(
    Placement(transformation(extent = {{-140, -60}, {-100, -20}})));
  Modelica.Blocks.Interfaces.RealInput Q_losses "Pertes thermiques [W]" annotation(
    Placement(transformation(extent = {{-140, -100}, {-100, -60}})));
  // --- Sorties
  Modelica.Blocks.Interfaces.BooleanOutput besoinON "True when tank requests heating (hysteresis)" annotation(
    Placement(transformation(extent = {{100, 60}, {140, 100}})));
  Modelica.Blocks.Interfaces.RealOutput Q_req_th "Besoin thermique brut du ballon [W]" annotation(
    Placement(transformation(extent = {{100, 20}, {140, 60}})));
  Modelica.Blocks.Interfaces.RealOutput Q_req "Puissance demandée à la PAC, limitée hydrauliquement [W]" annotation(
...
equation
  besoinON = on;
// Hystérésis
  on = if T_top < (T_set - dT_hys) then true else if T_top > (T_set + dT_hys) then false else pre(on);
...
<li>Convient pour les schémas ECS ou stockage tampon avec gestion par bande de température haute.</li>
    ...
    Icon(graphics = {Rectangle(origin = {-2, 0}, fillColor = {85, 255, 255}, fillPattern = FillPattern.Cross, extent = {{-104, 100}, {104, -100}}), Text(origin = {-24, 143}, textColor = {0, 0, 255}, extent = {{-132, 31}, {132, -31}}, textString = "%name")}, coordinateSystem(extent = {{-160, 180}, {140, -100}})),
    Diagram(coordinateSystem(extent = {{-140, 140}, {140, -100}})),
    Documentation(info = "<html><head></head><body>
<p><br></p><ul>
</ul>
</body></html>"));
end TankPowerRequest_Tset;
