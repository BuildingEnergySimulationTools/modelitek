...
algorithm
  ...
  if M[1,4] == 0 then M[1,4] := M[2,4] * Cnnav[3]; end if;
  if M[3,4] == 0 then M[3,4] := M[2,4] * Cnnav[1]; end if;
  if M[4,4] == 0 then M[4,4] := M[3,4] * Cnnav[2]; end if;
  if M[5,4] == 0 then M[5,4] := M[4,4] * Cnnav[4]; end if;

  // 3. Fill rows based on M[i,4]
  for i in 1:5 loop
    if M[i,2] == 0 then M[i,2] := M[i,4] * Cnnam[1]; end if;
    if M[i,3] == 0 then M[i,3] := M[i,4] * Cnnam[2]; end if;
    if M[i,5] == 0 then M[i,5] := M[i,4] * Cnnam[3]; end if;
    if M[i,1] == 0 then M[i,1] := M[i,2] * Cnnam[4]; end if;
  end for;
