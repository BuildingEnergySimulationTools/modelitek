...
model DualSourceSwitch "Simplified 3-way valve selecting between two sources"
  ...
  replaceable package Medium = Modelica.Media.Interfaces.PartialMedium 
    ...
    annotation (choices(
      choice(redeclare package Medium = Buildings.Media.Air "Air"),
      choice(redeclare package Medium = Buildings.Media.Water "Water"),
      choice(redeclare package Medium = Buildings.Media.Antifreeze.PropyleneGlycolWater(property_T=220, X_a=0.40)
  ...
  Modelica.Blocks.Interfaces.RealInput y(min=0, max=1) 
    "Actuator position (0=use source 1, 1=use source 2)"
    annotation (Placement(transformation(origin={-4,184}, extent={{-20,-20},{20,20}}, rotation=270),
                iconTransformation(origin={-40,28}, extent={{-20,-20},{20,20}}, rotation=270)));

  // === Fluid ports ===
  Modelica.Fluid.Interfaces.FluidPort_a port_a(redeclare package Medium=Medium) 
    "Common inlet (base)"
    annotation (Placement(transformation(extent = {{-114, 10}, {-94, 30}}), iconTransformation(origin = {14, -34}, extent = {{-114, 10}, {-94, 30}})));

  Modelica.Fluid.Interfaces.FluidPort_a port_a1(redeclare package Medium=Medium) 
    "Inlet source 1"
    annotation (Placement(transformation(extent = {{74, -182}, {94, -162}}), iconTransformation(origin = {-14, 80}, extent = {{74, -182}, {94, -162}})));

  Modelica.Fluid.Interfaces.FluidPort_a port_a2(redeclare package Medium=Medium) 
    "Inlet source 2"
    annotation (Placement(transformation(extent = {{74, 22}, {94, 42}}), iconTransformation(origin = {-14, -30}, extent = {{74, 22}, {94, 42}})));

  Modelica.Fluid.Interfaces.FluidPort_b port_b(redeclare package Medium=Medium) 
    "Common outlet (base)"
    annotation (Placement(transformation(extent = {{-114, -92}, {-94, -72}}), iconTransformation(origin = {14, 34}, extent = {{-114, -92}, {-94, -72}})));

  Modelica.Fluid.Interfaces.FluidPort_b port_b1(redeclare package Medium=Medium) 
    "Outlet 1 (connected to source 1 path)"
    annotation (Placement(transformation(extent = {{74, -102}, {94, -82}}), iconTransformation(origin = {-14, 32}, extent = {{74, -102}, {94, -82}})));

  Modelica.Fluid.Interfaces.FluidPort_b port_b2(redeclare package Medium=Medium) 
    "Outlet 2 (connected to source 2 path)"
    annotation (Placement(transformation(extent = {{74, 108}, {94, 128}}), iconTransformation(origin = {-14, -80}, extent = {{74, 108}, {94, 128}})));

  // === Internal valves ===
  Buildings.Fluid.Actuators.Valves.TwoWayEqualPercentage v_a_to_b1(
    redeclare package Medium=Medium, m_flow_nominal=m_flow_nominal, dpValve_nominal=dpValve_nominal);
  Buildings.Fluid.Actuators.Valves.TwoWayEqualPercentage v_a_to_b2(
    redeclare package Medium=Medium, m_flow_nominal=m_flow_nominal, dpValve_nominal=dpValve_nominal);
  Buildings.Fluid.Actuators.Valves.TwoWayEqualPercentage v_a1_to_b(
    redeclare package Medium=Medium, m_flow_nominal=m_flow_nominal, dpValve_nominal=dpValve_nominal);
  Buildings.Fluid.Actuators.Valves.TwoWayEqualPercentage v_a2_to_b(
    redeclare package Medium=Medium, m_flow_nominal=m_flow_nominal, dpValve_nominal=dpValve_nominal);
