...
block TriggeredAddReal "Add input to previous value of output, if rising edge of trigger port"
  ...
  parameter Boolean use_set = false "= true, if set port enabled and used as default value when reset" annotation(
    ...
    HideResult = true,
    choices(checkBox = true));
  parameter Real y_start = 0 "Initial and reset value of y if set port is not used";
  ...
  Modelica.Blocks.Interfaces.BooleanInput reset if use_reset annotation(
    Placement(transformation(extent = {{-20, -20}, {20, 20}}, rotation = 90, origin = {60, -120})));
  Modelica.Blocks.Interfaces.RealInput set if use_set annotation(
...
protected
  Modelica.Blocks.Interfaces.BooleanOutput local_reset annotation(
    HideResult = true);
  Modelica.Blocks.Interfaces.RealOutput local_set;
initial equation
  pre(y) = y_start;
equation
  if use_reset then
  ...
  annotation(
    Icon(coordinateSystem(preserveAspectRatio = false, extent = {{-100, -100}, {100, 100}}, initialScale = 0.06), graphics = {Line(points = {{-100, 0}, {32, 76}}, color = {255, 128, 0}, pattern = LinePattern.Dot), Line(points = {{-100, 0}, {32, -20}}, color = {255, 128, 0}, pattern = LinePattern.Dot), Line(points = {{-54, -56}, {-26, -56}, {-26, -20}, {32, -20}, {32, 76}}), Line(points = {{-60, -100}, {32, -20}}, color = {255, 0, 255}, pattern = LinePattern.Dot), Text(visible = use_reset, extent = {{-28, -62}, {94, -86}}, textString = "reset")}),
