...
model InjectEnergy
  ...
  // --- Inputs
  Modelica.Blocks.Interfaces.RealInput P_injected(unit = "W") "Injected load (W)" annotation(
    Placement(transformation(origin = {-164, -78}, extent = {{20, -20}, {-20, 20}}, rotation = -180), iconTransformation(origin = {-34, -4}, extent = {{30, -82}, {-10, -42}}, rotation = -180)));
  // --- Pump
  ...
  Modelica.Blocks.Interfaces.RealInput seasonMode(unit = "W") "Seasonal mode selector: Heating=1, Cooling=2, Off=3" annotation(
    Placement(transformation(origin = {-182, 22}, extent = {{20, -20}, {-20, 20}}, rotation = -180), iconTransformation(origin = {-32, -122}, extent = {{30, -82}, {-10, -42}}, rotation = -180)));
  ...
  Modelica.Blocks.Interfaces.RealInput Q_flow(unit = "W") "kg/s" annotation(
    Placement(transformation(origin = {-162, -119}, extent = {{20, -20}, {-20, 20}}, rotation = -180), iconTransformation(origin = {-34, -67}, extent = {{30, -82}, {-10, -42}}, rotation = -180)));
protected
...
// Pump circulation with controlled flow
  connect(T1.port_b, pump.port_a);
  connect(pump.port_b, T2.port_a);
...
// Pump flow logic
  connect(P_injected, demandOK.u1) annotation(
...
// Heat flow safety switch
  connect(P_injected, safeSwitch.u1) annotation(
...
<h4>InjectEnergy</h4>
<p>This model injects energy into a fluid stream according to an external power injection&nbsp;<code>P_injected</code>.</p>
<ul>
<li><b>SeasonMode input</b>: controls operation mode:
  <ul>
    <li><b>Heating</b> (1): the requested power is injected as heat gain.</li>
    <li><b>Cooling</b> (0): the requested power is&nbsp;injected&nbsp;as cooling.</li>
    <li><b>MidSeason</b>&nbsp;(0.5): same than heating.</li>
  </ul></li>
<li><b>Pmin logic</b>: if the requested power&nbsp;<span style=\"font-family: monospace;\">P_injected</span>&nbsp;is below <code>Pmin</code>, the pump enforces zero flow. Otherwise, the pump enforces a constant flow <code>Q_flow</code>.</li>
...
<ul>
  ...
  <li><code>Pmin</code>: minimum injected power to activate the pump [W]</li>
  <li><code>Q_flow</code>: fixed flow rate enforced when injection is above threshold [kg/s]</li>
</ul><div><br></div>
</body></html>"));
end InjectEnergy;
